...
# Класс исключения для обработки ошибок с размерами матрицы
class MatrixSizeError(Exception):
    def __init__(self, message):
        self.message = message
        super().__init__(self.message)

# Класс исключения для обработки ошибок при сложении матриц
class MatrixAdditionError(Exception):
    def __init__(self, message):
        self.message = message
        super().__init__(self.message)

# Класс исключения для обработки ошибок при умножении матриц
...
try:
    ...
    matrix2 = Matrix(2, 3)  # Вызывает исключение MatrixSizeError
    ...
    print(matrix1 == matrix2)  # Вызывает исключение MatrixSizeError
    
    result = matrix1 + matrix3  # Вызывает исключение MatrixAdditionError
    
    result = matrix4 * matrix5  # Вызывает исключение MatrixMultiplicationError
